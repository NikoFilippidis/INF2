...
public class GeneratorSeen {
	...
	public static Karte generateSeen() {
	...
	private static void generate(Karte k) {
...
//			int steppeHoeheUnten = (randomZahl.nextInt(zehntelallFelderDurchX) + 1)+(randomZahl.nextInt(3));
			...
			for (int q = (Parameter.spielfeldAnzahlY - randomZahlUnten); q > (Parameter.spielfeldAnzahlY
				...
				if (r >= 25) {
					if (k.getFelder()[i][q+1] instanceof Eis) {
						...
						if(i>=2&&i>Parameter.spielfeldAnzahlX){
			...
			wuesteAusschlagUnten = wuesteAusschlagUnten + randomZahl.nextInt(3)-1;
			...
			int steppeRandUnten = randomZahl.nextInt(2) + 2;
			steppeRandUnten = steppeRandUnten + randomZahl.nextInt(3) - 1;
			if (steppeRandUnten == 0) 
				steppeRandUnten = 1;
			if (steppeRandUnten == wuesteXYAusschlagMax + 1)
				steppeRandUnten = wuesteXYAusschlagMax;
			...
			wuesteAusschlagOben = wuesteAusschlagOben + randomZahl.nextInt(3)-1;
			...
			int steppeRandOben = randomZahl.nextInt(3) + 1;
			steppeRandOben = steppeRandOben + randomZahl.nextInt(3) - 1; 
			if (steppeRandOben == 0)
				steppeRandOben = 1;
			if (steppeRandOben == wuesteXYAusschlagMax + 1)
				steppeRandOben = wuesteXYAusschlagMax;
	...
	private static void rest(Karte k) {
		  int zufall=0;
		  int zufallMuster=0;
		  for (int i = 1; i <= Parameter.spielfeldAnzahlX; i++) {
		   for (int j = 1; j < Parameter.spielfeldAnzahlY; j++) {
		    zufall = randomZahl.nextInt(6)+1;
		    zufallMuster = randomZahl.nextInt(6)+1;
		    if(k.getFelder()[i][j]==null){
		     switch (zufall) {
		     case 1:
		    	 //Berg
		    	 if (i<(Parameter.spielfeldAnzahlX)) {
			    		 ...
			    		 //Es gibt 6 verschiedene Muster eines wird zufaellig gewaehlt
					      ...
					      case 1:	
							if(k.getFelder()[i+1][j]==null&&k.getFelder()[i][j+1]==null&&k.getFelder()[i+1][j+1]==null){
						       ...
						       k.getFelder()[i+1][j]= new Sumpf();
						       k.getFelder()[i][j+1]= new Sumpf();
						       k.getFelder()[i+1][j+1]= new Sumpf();
						      }else
						       k.getFelder()[i][j] = new Sumpf();
							break;
					      case 2:
	...
	private static void seenGenerieren(Karte k) {
		int inselanzahl = (((Parameter.spielfeldAnzahlX + Parameter.spielfeldAnzahlY) / 2) / 10);
		int mindestgrosse = (((Parameter.spielfeldAnzahlX + Parameter.spielfeldAnzahlY) / 2) / 10)/2;
		int teiler = 2;
		int[] randomrechts = new int[inselanzahl];
		int[] randomlinks = new int[inselanzahl];
		int[] randomunten = new int[inselanzahl];
		int[] randomoben = new int[inselanzahl];
		int[] x = new int[inselanzahl];
		int[] y = new int[inselanzahl];
		int ungultig = 0;

		for (int j = 0; j < inselanzahl; j++) {
			do {
				ungultig = 0;
				randomrechts[j] = r.nextInt(Parameter.spielfeldAnzahlX / 10/teiler)
						+ mindestgrosse;
				randomlinks[j] = r.nextInt(Parameter.spielfeldAnzahlX / 10/teiler)
						+ mindestgrosse;
				randomunten[j] = r.nextInt(Parameter.spielfeldAnzahlY / 10/teiler)
						+ mindestgrosse;
				randomoben[j] = r.nextInt(Parameter.spielfeldAnzahlY / 10/teiler)
						+ mindestgrosse;
				x[j] = r.nextInt(Parameter.spielfeldAnzahlX - 5
						- randomlinks[j] - randomrechts[j])
						+ randomlinks[j]+3;
				y[j] = r.nextInt(Parameter.spielfeldAnzahlY - maxeis * 2
						- randomunten[j] - randomoben[j] - 3)
						+ maxeis + randomoben[j] + 1;

				for (int i = y[j] + randomunten[j] + 2; i >= y[j]
						- randomoben[j] - 2; i--) {
					for (int g = x[j] - randomlinks[j]-2; g <= x[j]
							+ randomrechts[j] + 2; g++) {
						if (k.getFelder()[g][i] ==null || k.getFelder()[g][i] instanceof Wueste || k.getFelder()[g][i] instanceof Steppe) {
						} else {
		...
		while (wertY < unten) {
			...
			if (wertX == Xwert - 1) {
				while (wertY < unten) {
					...
					k.getFelder()[wertX][wertY] = new Meer();
					k.getFelder()[wertX - 1][wertY] = new Kueste();
					k.getFelder()[wertX - 1][wertY + 1] = new Kueste();
					k.getFelder()[wertX][wertY + 1] = new Kueste();
