...
public class generatorinselTest {
...
/*	@Test
	public void RichtigeAnzahlSpieler(){
		...
		assertTrue(k.getFelder()[i][j].getSpielerstart()>1&&k.getFelder()[i][j].getSpielerstart()<=4);
			}
		}		
		} */
	
	@Test
		public void mindestanzahlinsel(){
		Parameter.spielfeldAnzahlX = (byte)100;
		Parameter.spielfeldAnzahlY = (byte)100;
		int anzahl = generatorinsel.inselanzahl;
		if(anzahl >= ((Parameter.spielfeldAnzahlX + Parameter.spielfeldAnzahlY) / 2) / 10){
			assertTrue(true);
		}
		else{
			assertTrue(false);
	...
	@Test
	public void istMeerOderKuesteZwischenInseln() {
		Parameter.spielfeldAnzahlX = (byte) 100;
		Parameter.spielfeldAnzahlY = (byte) 100;
