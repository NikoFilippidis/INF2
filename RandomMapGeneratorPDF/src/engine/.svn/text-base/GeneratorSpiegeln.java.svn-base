...
public class GeneratorSpiegeln {
	...
	public static Karte generateinsel() {
		...
		for (int i = 1; i <= Parameter.spielfeldAnzahlX / 2; i++) {
			for (int j = 2; j <= Parameter.spielfeldAnzahlY / 2; j++) {
	...
	private static void spiegeln(Karte k) {
		
		
		for (int i = 1; i <= Parameter.spielfeldAnzahlX / 2; i++) {
			for (int j = 1; j <= Parameter.spielfeldAnzahlY / 2; j++) {
				k.getFelder()[Parameter.spielfeldAnzahlX + 1 - i][j] = k
						.getFelder()[i][j];
			}
		}
		...
		for (int i = 1; i <= Parameter.spielfeldAnzahlX; i++) {
			for (int j = 1; j <= Parameter.spielfeldAnzahlY / 2; j++) {
				k.getFelder()[i][Parameter.spielfeldAnzahlY + 1 - j] = k
						.getFelder()[i][j];
			}
		}
		for (int i = 1; i <= Parameter.spielfeldAnzahlY; i++) {
			if (Parameter.spielfeldAnzahlX % 2 != 0) {
				k.getFelder()[Parameter.spielfeldAnzahlX / 2 - 1][i] = new Kueste();
			}
			k.getFelder()[Parameter.spielfeldAnzahlX / 2][i] = new Kueste();
			k.getFelder()[Parameter.spielfeldAnzahlX / 2 + 1][i] = new Kueste();
		}
		for (int i = 1; i <= Parameter.spielfeldAnzahlX; i++) {
			if (Parameter.spielfeldAnzahlY % 2 == 0) {
				k.getFelder()[i][Parameter.spielfeldAnzahlY / 2] = new Kueste();
			}
			k.getFelder()[i][Parameter.spielfeldAnzahlY / 2 + 1] = new Kueste();
		}
		for (int i = 1; i <= Parameter.spielfeldAnzahlX / 2; i++) {
			for (int j = 1; j <= Parameter.spielfeldAnzahlY / 2; j++) {
	...
	private static void diagonalenFuellerWueste(Karte k) {
		for (int i = 2; i <= Parameter.spielfeldAnzahlX - 1 / 2; i++) {
			for (int j = 2; j <= Parameter.spielfeldAnzahlY - 1 / 2; j++) {
				if (k.getFelder()[i][j] instanceof Meer) {
					if (!(k.getFelder()[i - 1][j - 1] instanceof Meer)
							&& !(k.getFelder()[i - 1][j - 1] instanceof Kueste)
	...
	private static void seenDiagonaleKorrektur(Karte k) {
		for (int i = 2; i <= Parameter.spielfeldAnzahlX / 2 - 1; i++) {
			for (int j = 2; j <= Parameter.spielfeldAnzahlY / 2 - 1; j++) {
				if (k.getFelder()[i][j] instanceof Wueste) {
					if (!(k.getFelder()[i - 1][j - 1] instanceof Wueste)
							&& !(k.getFelder()[i - 1][j - 1] instanceof Kueste)
							&& !(k.getFelder()[i - 1][j - 1] instanceof Steppe)
							&& !(k.getFelder()[i - 1][j - 1] instanceof Meer)) {
	...
	private static void spielersetzen(Karte k) {
		...
		if ((Parameter.spielfeldAnzahlX + Parameter.spielfeldAnzahlY) % 2 != 0) {
			if (Parameter.spielfeldAnzahlY % 2 != 0) {
				k.getFelder()[x[0]][y[0]].setSpielerstart(1);
				k.getFelder()[x[0]+ Parameter.spielfeldAnzahlX / 2 + 1][y[0]]
						.setSpielerstart(2);
				k.getFelder()[x[0]][y[0]+ Parameter.spielfeldAnzahlY / 2]
						.setSpielerstart(3);
				k.getFelder()[x[0]+ Parameter.spielfeldAnzahlX / 2 + 1][y[0]
						+ Parameter.spielfeldAnzahlY / 2].setSpielerstart(4);
			}
			if (Parameter.spielfeldAnzahlX % 2 != 0) {
				k.getFelder()[x[0]][y[0]].setSpielerstart(1);
				k.getFelder()[x[0] + Parameter.spielfeldAnzahlX / 2][y[0]]
						.setSpielerstart(2);
				k.getFelder()[x[0]][y[0]+ Parameter.spielfeldAnzahlY / 2]
						.setSpielerstart(3);
				k.getFelder()[x[0]+ Parameter.spielfeldAnzahlX / 2][y[0]
						+ Parameter.spielfeldAnzahlY / 2 + 1]
						.setSpielerstart(4);
			}
			if (Parameter.spielfeldAnzahlX % 2 != 0
					&& Parameter.spielfeldAnzahlY % 2 != 0) {
				k.getFelder()[x[0 + Parameter.spielfeldAnzahlX / 2]][y[0]
						+ Parameter.spielfeldAnzahlY / 2].setSpielerstart(4);
			}
		} else {
			...
			k.getFelder()[x[0]+ Parameter.spielfeldAnzahlX / 2 + 1][y[0]]
					.setSpielerstart(2);
			k.getFelder()[x[0]][y[0]+ Parameter.spielfeldAnzahlY / 2 + 1]
					.setSpielerstart(3);
			k.getFelder()[x[0] + Parameter.spielfeldAnzahlX / 2 + 1][y[0]
					+ Parameter.spielfeldAnzahlY / 2 + 1].setSpielerstart(4);
		}
	}

	/*
	 * //f�r euch !!! ja gut die oberen 2 for schleifen braucht ihr auch noch
	 * f�r pangea m�ssen noch randwerte ge�ndert werden und eine randomzahl
	 * generiert werden, um damit so und so viele felder in pangea reinzugehen
	 */
	...
	private static int zzz(int a, int e) {
		int zahl;
		do {
			zahl = r.nextInt(Parameter.spielfeldAnzahlY / 2);
		} while (zahl < a || zahl > e);
		return zahl;
	}
	...
	private static void wuestegenerieren(Karte k) {
		...
		for (int i = 1; i <= Parameter.spielfeldAnzahlX; i++) {
			if (k.getFelder()[i][Parameter.spielfeldAnzahlY] == null) {
		...
		for (int i = 1; i <= Parameter.spielfeldAnzahlX / 2; i++) {
			for (int j = 1; j < Parameter.spielfeldAnzahlY / 2; j++) {
				...
				if (k.getFelder()[i][j] == null) {
					...
					case 1:
						// Berg
						if (i < (Parameter.spielfeldAnzahlX / 2 - 1)) {
							// Es gibt 6 verschiedene Muster eines wird
							// zufaellig gewaehlt
							switch (zufallMuster) {
							case 1:
								if (k.getFelder()[i + 1][j] == null
										&& k.getFelder()[i][j + 1] == null
										&& k.getFelder()[i + 1][j + 1] == null) {
	...
	private static void inselgenerieren(Karte k) {
		...
		int mindestgrosse = (((Parameter.spielfeldAnzahlX + Parameter.spielfeldAnzahlY) / 2) / 10) / 2;
		...
		for (int j = 0; j < inselanzahl; j++) {
			do {
				...
				randomoben[j] = r.nextInt(Parameter.spielfeldAnzahlY / 10)
						+ mindestgrosse;
				x[j] = r.nextInt(Parameter.spielfeldAnzahlX / 2 - 5
						- randomlinks[j] - randomrechts[j])
						+ randomlinks[j] + 3;
				y[j] = r.nextInt(Parameter.spielfeldAnzahlY / 2 - maxeis
